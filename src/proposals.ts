 ...
 *   pending ──sweep()────▶ expired       (10 minutes after creation, logged)
 ...
 *
 * The store logs `proposed` when a proposal is created, `expired` when a
 * sweep finds one past its deadline, and `approved` from `approve()`. The
 * `executed` / `failed` entries come from the execute helpers of the
...
export type SettledStatus = 'approved' | 'expired' | 'stale' | 'refused';
export type ProposalStatus = 'pending' | SettledStatus;

export type ProposalLookup =
  | { status: 'unknown' }
  | { status: 'pending'; proposal: Proposal }
  | { status: SettledStatus; proposal: Proposal; settledAt: Date };
...
export class ProposalStore {
  ...
  create(input: CreateProposalInput): Proposal {
    ...
    this.records.set(proposal.id, { proposal, status: 'pending', settledAt: null });
    return proposal;
  }
  ...
  lookup(id: string): ProposalLookup {
    ...
    return {
      ...
      settledAt: record.settledAt ?? record.proposal.expiresAt,
    };
  }
